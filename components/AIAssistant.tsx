...
export const AIAssistant: React.FC<{ lang?: 'ar' | 'en' }> = ({ lang = 'ar' }) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const aiText = await generateText(userMsg, {
        model: 'gemini-3-flash-preview',
        systemInstruction: `أنت "المعلم الخصوصي الذكي". المرجع: ${contextText.substring(0, 15000)}\nتعرف تلقائياً على لغة الطالب وأجب بنفس لغة ملفه بأسلوب تعليمي مبسط جداً ومبهر.`
      });
  ...
  const generateCards = async () => {
    ...
    try {
      ...
      const cards: Flashcard[] = JSON.parse(text || '[]');
      
      const cardsWithImages = await Promise.all(cards.map(async (card) => {
        try {
          const imageUrl = await generateImage(card.imagePrompt, { aspectRatio: '1:1' });
  ...
  const generateInfographic = async () => {
    ...
    try {
      ...
      const text = await generateText(prompt, { model: 'gemini-3-flash-preview', responseMimeType: 'application/json' });
      const data: InfographicData = JSON.parse(text || '{}');

      const processItems = async (items: any[]) => {
        return await Promise.all(items.map(async (it) => {
          try {
            const imageUrl = await generateImage(it.imagePrompt, { aspectRatio: '1:1' });
            return { ...it, imageUrl };
          } catch (e) { return it; }
        }));
      };
...
النص المرجعي: ${context.substring(0, 15000)}`;
      ...
      const responsePromise = generateText(prompt, { model: 'gemini-3-flash-preview', responseMimeType: 'application/json' });
      ...
      const text = await Promise.race([responsePromise, timeoutPromise]) as any;
