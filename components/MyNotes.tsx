...
import { generateFromAudio, generateText, textToSpeech } from '../services/geminiService';
...
export const MyNotes: React.FC<{ lang?: 'ar' | 'en' }> = ({ lang = 'ar' }) => {
  ...
  const handleManualTranslate = async (note: Note) => {
    ...
    try {
      ...
      const translation = await generateText(prompt, { model: 'gemini-3-flash-preview' });
  ...
  const speakNote = async (text: string) => {
    try {
      const audioData = await textToSpeech(`اقرأ الملاحظة بوضوح: ${text}`, 'Kore');
      if (audioData) {
