...
export const ImageEditor: React.FC<{ lang?: 'ar' | 'en' }> = ({ lang = 'ar' }) => {
  ...
  const processAI = async (customPrompt?: string) => {
    ...
    try {
      const requestPrompt = sourceImage
        ? `Modify this image: ${finalPrompt}. Maintain consistent objects but apply the requested changes. Return only the image.`
        : `Create a high-quality educational illustration: ${finalPrompt}. No text on image, professional style.`;

      const imgBase64 = sourceImage ? sourceImage.split(',')[1] : undefined;
      const imgMime = sourceImage ? 'image/png' : undefined;

      const imageDataUrl = await generateImage(requestPrompt, {
